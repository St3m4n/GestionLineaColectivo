...
import type { FullState } from './server/types.js';
...
app.get('/api/state', (_request: FastifyRequest, reply: FastifyReply) => {
  try {
...
app.put('/api/state', (request: FastifyRequest, reply: FastifyReply) => {
  try {
    const body = request.body as FullState;
...
app.get('/api/ventas/:year/:month', (request: FastifyRequest, reply: FastifyReply) => {
  const params = request.params as { year: string; month: string };
  const ventas = sales.getVentasShard(params.year, params.month);
  return reply.send(ventas);
});

app.post('/api/ventas/tarjeta', (request: FastifyRequest, reply: FastifyReply) => {
  try {
