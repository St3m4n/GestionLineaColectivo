...
const shardFor = (dateOnly) => {
    const [year, month] = dateOnly.split('-');
    return `ventas_${year}_${month}.json`;
};
const getDateOnlyFromIso = (iso) => normalizeDateOnly(iso.slice(0, 10));
export class SalesService {
    ...
    getVehiculos() {
        return this.store.readArrayFile(VEHICULOS_FILE);
    }
    saveVehiculos(data) {
        this.store.writeArrayFileAtomic(VEHICULOS_FILE, data);
    }
    getConductores() {
        return this.store.readArrayFile(CONDUCTORES_FILE);
    }
    getMultas() {
        return this.store.readArrayFile(MULTAS_FILE);
    }
    readTarjetasAllShards() {
        const files = this.store.listFilesByPrefix('ventas_');
        if (files.length === 0)
            return [];
        const all = [];
        for (const file of files) {
            const items = this.store.readArrayFile(file);
            all.push(...items);
        }
        return all.sort((a, b) => new Date(a.fechaEmision).getTime() - new Date(b.fechaEmision).getTime());
    }
    writeTarjetasSharded(tarjetas) {
    ...
    getFullState() {
        const storedPrintSettings = this.store.readObjectFile(PRINT_SETTINGS_FILE, DEFAULT_PRINT_SETTINGS);
        return {
            vehiculos: this.getVehiculos(),
            conductores: this.getConductores(),
            tarjetas: this.readTarjetasAllShards(),
            multas: this.getMultas(),
            controlador: this.store.readObjectFile(CONTROLADOR_FILE, DEFAULT_CONTROLADOR),
            auditoriaDesbloqueos: this.store.readArrayFile(AUDITORIA_DESBLOQUEOS_FILE),
            auditoriaAsignaciones: this.store.readArrayFile(AUDITORIA_ASIGNACIONES_FILE),
            printSettings: {
                ...DEFAULT_PRINT_SETTINGS,
                ...storedPrintSettings,
            },
            diasNoHabiles: this.store.readArrayFile(DIAS_NO_HABILES_FILE),
            asignacionesR: this.store.readObjectFile(ASIGNACIONES_R_FILE, {}),
        };
    }
    saveFullState(payload) {
    ...
    getDynamicVariacion(vehiculoId, vehiculos, asignacionesR, baseDate) {
        ...
        if (asignacionesR && Object.prototype.hasOwnProperty.call(asignacionesR, dateStr)) {
            ...
            if (troncales.length > 0) {
                ...
                for (let i = 0; i < 8; i++) {
                    if (troncales[(startIdx + i) % troncales.length].id === vehiculoId)
                        return 'R';
                }
            }
            if (variante2s.length > 0) {
                ...
                for (let i = 0; i < 8; i++) {
                    if (variante2s[(startIdx + i) % variante2s.length].id === vehiculoId)
                        return 'R';
                }
            }
        }
    ...
    getVentasShard(year, month) {
        const shardName = `ventas_${year}_${month.padStart(2, '0')}.json`;
        return this.store.readArrayFile(shardName);
    }
    venderTarjeta(payload) {
        const vehiculos = this.getVehiculos();
        const conductores = this.getConductores();
        const multas = this.getMultas();
        const asignacionesR = this.store.readObjectFile(ASIGNACIONES_R_FILE, {});
        ...
        if (!conductor)
            return { success: false, message: 'No hay conductor asociado a este vehículo' };
        ...
        const multasVencidas = multas.filter(m => m.vehiculoId === payload.vehiculoId && !m.pagada && m.fechaVencimiento && new Date(`${m.fechaVencimiento}T12:00:00`).getTime() < Date.now());
        ...
        for (const [shard, dates] of datesByShard) {
            const current = this.store.readArrayFile(shard);
            ...
            if (duplicate)
                return { success: false, message: 'Una o más fechas ya tienen una tarjeta emitida.' };
        }
        const createdByShard = new Map();
        const now = new Date();
        const cards = targetDates.map((dateOnly, index) => {
        ...
        const updatedVehiculos = vehiculos.map(v => v.id === payload.vehiculoId
            ? {
                ...v,
                estadoCuenta: {
                    ...v.estadoCuenta,
